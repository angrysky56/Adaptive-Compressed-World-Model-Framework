...
const KnowledgeForm = ({ showNotification }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (inputMode === 'text') {
        ...
        response = await axios.post('/api/add_knowledge', {
          text: text,
        ...
        response = await axios.post('/api/upload_file', formData, {
          headers: {
      ...
      if (response.data.success) {
        ...
        if (response.data.entities_extracted && response.data.entities_extracted.length > 0) {
          setEntities(response.data.entities_extracted);
        }
        
        showNotification('Knowledge added successfully', 'success');
      } else {
        showNotification(response.data.message || 'Failed to add knowledge', 'error');
      }
    } catch (error) {
      console.error('Error adding knowledge:', error);
      showNotification(error.response?.data?.message || error.message || 'Failed to add knowledge', 'error');
    } finally {
      setLoading(false);
    }
  };
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
        ...
        >
          ...
          <input
            type="file"
            hidden
            accept=".txt,.md,.csv,.json,.xml,.html,.doc,.docx,.pdf"
